...
class ObjectBuilder extends AbstractObjectBuilder
{
     ...
     *
     * @see AbstractOMBuilder::getNamespace
     */
            ...
            try {
                if (
                    !($this->getPlatform() instanceof MysqlPlatform
                      && ('0000-00-00 00:00:00' === $val || '0000-00-00' === $val))
                ) {
...
{';
    ...
    protected function addClassBody(string &$script): void
    {
        ...
        if ($baseClass && false !== strrpos($baseClass, '\\')) {
...
';
    ...
    protected function addColumnAttributes(string &$script): void
    {
        ...
        foreach ($table->getColumns() as $col) {
            ...
            if (PropelTypes::OBJECT == $col->getType() || PropelTypes::PHP_ARRAY == $col->getType()) {
...
';
    ...
    protected function addHookMethods(string &$script): void
    {
        ...
        $hooks['hasBaseClass'] = null !== $this->getBehaviorContent('parentClass') || null !== $className;
...
';
    ...
    protected function addHasArrayElement(string &$script, Column $column): void
    {
        ...
        $columnType      = (PropelTypes::PHP_ARRAY === $column->getType()) ? 'array' : 'set';
        $script          .= "
    /**
...
';
    ...
    protected function addLazyLoaderBody(string &$script, Column $column): void
    {
        ...
        if (PropelTypes::BLOB === $column->getType() && $platform instanceof SqlsrvPlatform) {
            $script .= '
...
';
        ...
        if (PropelTypes::CLOB === $column->getType() && $platform instanceof OraclePlatform) {
        ...
        } elseif (PropelTypes::UUID_BINARY === $column->getType()) {
...
';
    ...
    protected function addAddArrayElement(string &$script, Column $col): void
    {
        ...
        $columnType      = (PropelTypes::PHP_ARRAY === $col->getType()) ? 'array' : 'set';
        $script          .= "
    /**
...
';
    ...
    protected function addHasOnlyDefaultValuesBody(string &$script): void
    {
        ...
        foreach ($colsWithDefaults as $col) {
            ...
            if (0 === strpos($defaultValueString, 'new ')) {
    ...
    protected function addHydrateBody(string &$script): void
    {
        ...
        foreach ($table->getColumns() as $col) {
            ...
            \$col = \$row[{$indexName}];";
                ...
                if (PropelTypes::CLOB_EMU === $col->getType() && $this->getPlatform() instanceof OraclePlatform) {
                    // PDO_OCI returns a stream for CLOB objects, while other PDO adapters return a string...
                    $script .= "
                ...
                \$this->{$clo} = null;
                    ...
                    if ($this->getPlatform() instanceof MysqlPlatform) {
                        if (in_array($col->getType(), [PropelTypes::TIMESTAMP, PropelTypes::DATETIME], true)) {
                            $handleMysqlDate        = true;
                            $mysqlInvalidDateString = '0000-00-00 00:00:00';
                        } elseif (PropelTypes::DATE === $col->getType()) {
                            $handleMysqlDate        = true;
                            $mysqlInvalidDateString = '0000-00-00';
                        }
                        // 00:00:00 is a valid time, so no need to check for that.
...
';
        ...
        foreach ($table->getCrossFks() as $crossFKs) {
            ...
            foreach ($crossFKs->getCrossForeignKeys() as $fk) {
                $collName = 'coll' . $this->getFKPhpNameAffix($fk, true);
                $script   .= '
            $this->' . $collName . ' = null;';
            }
        }
    ...
    protected function addFKAccessor(string &$script, ForeignKey $fk): void
    {
        ...
        foreach ($fk->getMapping() as $mapping) {
            ...
            if ($rightValueOrColumn instanceof Column) {
                ...
                if ('int' === $cptype || 'float' === $cptype || 'double' === $cptype) {
                    $conditional .= $and . '$this->' . $clo . ' != 0';
                } elseif ('string' === $cptype) {
...
";
    ...
    {
        /** @var ObjectCollection \${$inputCollection}ToDelete */
...
";
    ...
    protected function addCrossFKClear(string &$script, CrossForeignKeys $crossFKs): void
    {
        ...
        if ($multi) {
            $collName = 'combination' . ucfirst($this->getCrossFKsVarName($crossFKs));
            $lines    .= "\$this->{$collName} = null; // important to set this to NULL since that means it is uninitialized
";
        }
...
";
        ...
        $script .= "
    /**
     * Clears out the {$relCol} collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
...
";
    ...
    protected function addCrossFKSet(string &$script, CrossForeignKeys $crossFKs): void
    {
        ...
        if ($multi) {
            ...
            $collName               = 'coll' . $this->getFKPhpNameAffix($crossFK, true);
        }
...
';
    ...
    {';
        ...
        $criteria = $this->buildCriteria();';
            ...
            if (IdMethod::NO_ID_METHOD != $this->getTable()->getIdMethod()) {
...
';
    ...
    protected function addDoInsertBodyRaw(): string
    {
        ...
        } elseif (IdMethod::NATIVE == $table->getIdMethod() && (PlatformInterface::SEQUENCE == $platform->getNativeIdMethod() || PlatformInterface::SERIAL == $platform->getNativeIdMethod())) {
...
';
        ...
        if (!$platform->isNativeIdMethodAutoIncrement() && 'native' === $table->getIdMethod()) {
...
';
    ...
    public function clearAllReferences(bool $deep = false)
    {
        ...
        foreach ($this->getTable()->getCrossFks() as $crossFKs) {
            ...
            foreach ($crossFKs as $fk) {
                $varName = "coll{$this->getFKPhpNameAffix($fk, true)}";
