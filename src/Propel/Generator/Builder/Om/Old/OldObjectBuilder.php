 ...
 */
class OldObjectBuilder extends NewAbstractObjectBuilder
{
    ...
    {
        return 'Base' . $this->getStubObjectBuilder()->getUnprefixedClassName();
    }
...
{";
    ...
    protected function addClassBody(&$script)
    {
        ...
        $this->declareRepository($this->getTable());
        $this->addRepositoryPlaceholder($script);
        $this->addConstructorPlaceholder($script);
...
//
        ...
        $this->addRepositoryProperties($script);
...
//        $this->addMagicCall($script);
    ...
    protected function addRepositoryPlaceholder(&$script)
    {
        $script .= "\n{{propelRepositoryPlaceholder}}";
    }

    protected function addConstructorPlaceholder(&$script)
    {
        $script .= "\n{{propelConstructorPlaceholder}}";
    }
    ...
    protected function addConstructor(&$script)
    {
        ...
        foreach ($this->usedRepositories as $info) {
            $signature[] = $info['shortClassName'] . ' $' . $info['varName'] . ' = null';
            $phpDoc[] = '@param ' . $info['shortClassName'] . ' $' . $info['varName'];
            $setter[] = '$this->' . $info['varName'] . ' = $' . $info['varName'].';';
        }
...
";
        ...
        $script = preg_replace('/\{\{propelConstructorPlaceholder\}\}/', $constructor, $script, 1);
    }

    protected function addRepositoryProperties(&$script)
    {
        $properties = '';

        foreach ($this->usedRepositories as $repoInfo) {

            $properties .= "
    /**
     * The repository of [{$repoInfo['entityClassName']}] entity.
     * @var {$repoInfo['shortClassName']}
     */
    protected \${$repoInfo['varName']};
";
        }

        $script = preg_replace('/\{\{propelRepositoryPlaceholder\}\}/', $properties, $script, 1);
    ...
    protected function addMutatorCloseBody(&$script, Column $column)
    {
        ...
        if ($column->isForeignKey()) {
            ...
            foreach ($column->getForeignKeys() as $fk) {
                ...
                $script .= "
        if (\$this->$varName !== null && \$this->".$varName."->get".$colFK->getName()."() !== \$v) {
            \$this->$varName = null;
        }
";
            } // foreach fk
            ...
            if ( $tblFK->getName() != $table->getName() ) {
                ...
                foreach ($column->getForeignKeys() as $fk) {
                    ...
                    if ($refFK->isLocalPrimaryKey()) {
                        ...
                        $script .= "
        // update associated ".$tblFK->getName()."
        if (\$this->$varName !== null) {
            \$this->{$varName}->set".$colFK->getName()."(\$v);
        }
";
                    } else {
        ...
        if (\$this->$collName !== null) {
            foreach (\$this->$collName as \$referrerObject) {
                    \$referrerObject->set".$colFK->getName()."(\$v);
                }
            }
";
...
";
    ...
    protected function addHydrateBody(&$script)
    {
        ...
        foreach ($table->getColumns() as $col) {
            if (!$col->isLazyLoad()) {
                $indexName = "TableMap::TYPE_NUM == \$indexType ? $n + \$startcol : $tableMap::translateFieldName('{$col->getName()}', TableMap::TYPE_PHPNAME, \$indexType)";
...
";
    ...
    public function toArray(\$keyType = TableMap::$defaultKeyType, \$includeLazyLoadColumns = true, \$alreadyDumpedObjects = array()" . ($hasFks ? ", \$includeForeignObjects = false" : '') . ")
    {
        ...
        foreach ($this->getTable()->getColumns() as $num => $col) {
            if ($col->isLazyLoad()) {
                $script .= "
            \$keys[$num] => (\$includeLazyLoadColumns) ? \$this->get".$col->getName()."() : null,";
            } else {
                $script .= "
            \$keys[$num] => \$this->get".$col->getName()."(),";
            }
        }
        $script .= "
        );";
...
//        $this->addDoSave($script);
    ...
    {
        return \$this->get".$pkeys[0]->getName()."();
    }
";
...
";
    ...
    {";
        if (count($pkeys) == 1) {
            $script .= "
        return null === \$this->get" . $pkeys[0]->getName() . "();";
        } else {
            $tests = array();
            foreach ($pkeys as $pkey) {
                $tests[]= "(null === \$this->get" . $pkey->getName() . "())";
            }
            $script .= "
        return " . join(' && ', $tests) . ";";
        }
...
";
    ...
    {";
        foreach ($fk->getLocalColumns() as $columnName) {
            ...
            $script .= "
        if (\$v === null) {
            \$this->set".$column->getName()."(".$this->getDefaultValueString($column).");
        } else {
            \$this->set".$column->getName()."(\$v->get".$colFK->getName()."());
        }
";
...
";
    ...
    {";
        ...
        if ($findPk) {
            $script .= "
            {$this->getRepositoryAssignment($fk->getForeignTable(), '$repository', '            ')}
            \$this->$varName = \$repository->find($localColumns, \$con);";
        } else {
            $script .= "
            {$this->getRepositoryAssignment($fk->getForeignTable(), '$repository', '            ')}
            \$this->$varName \$repository->createQuery()
        ...
        if ($fk->isLocalPrimaryKey()) {
            ...
            $script .= "
            /*
             The following is used additionally to
             guarantee the related object contains a reference
             to this object.
            */
            \$this->{$varName}->add".$this->getRefFKPhpNameAffix($fk, false)."(\$this);
             ";
        }
...
";
    ...
    protected function addRefFKGetJoinMethods(&$script, ForeignKey $refFK)
    {
        ...
        foreach ($tblFK->getForeignKeys() as $fk2) {
            ...
            if ($this->getTable()->getName() == $tblFK2->getName()) {
                $doJoinGet = false;
            }
     ...
     * Otherwise if this ".$table->getName()." is new, it will return
     * an empty collection; or if this ".$table->getName()." has previously
     * been saved, it will retrieve related $relCol from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in ".$table->getName().".
     *
...
";
            foreach ($crossFKs->getIncomingForeignKey()->getColumnObjectsMapping() as $reference) {
                ...
                $script .= "
                        \$entryPk[$idx] = \$this->get{$foreign->getName()}();";
            }
            ...
            foreach ($crossFKs->getCrossForeignKeys() as $crossFK) {
                foreach ($crossFK->getColumnObjectsMapping() as $reference) {
                    ...
                    $idx = array_search($local, $crossPks, true);
                    $script .= "
                        \$entryPk[$idx] = \$combination[$combinationIdx]->get{$foreign->getName()}();";
                }
                $combinationIdx++;
            }
            ...
            foreach ($crossFKs->getUnclassifiedPrimaryKeys() as $pk) {
                ...
                $script .= "
                        //\$combination[$combinationIdx] = {$pk->getName()};
...
";
            ...
            foreach ($crossFK->getColumnObjectsMapping() as $reference) {
                ...
                $idx = array_search($local, $crossPks, true);
                $script .= "
                        \$entryPk[$idx] = \$entry->get{$foreign->getName()}();";
            }
...
";
            ...
            foreach ($crossFKs->getCrossForeignKeys() as $crossFK) {
                $script .= "
                    //\$combination[$combinationIdx] = {$crossFK->getForeignTable()->getName()} ({$crossFK->getName()})
            ...
            foreach ($crossFKs->getUnclassifiedPrimaryKeys() as $pk) {
                $script .= "
                    //\$combination[$combinationIdx] = {$pk->getName()}; Nothing to save.";
                    $combinationIdx++;
            }
...
";
        ...
        foreach ($crossFKs->getCrossForeignKeys() as $fk) {
            ...
            $filterName = $fk->getName();
            $name = lcfirst($fk->getName());
        ...
        foreach ($crossFKs->getUnclassifiedPrimaryKeys() as $pk) {
            $filterName = $pk->getName();
            $name = lcfirst($pk->getName());
    ...
    protected function addDoInsertBodyWithIdMethod()
    {
        ...
        foreach ($table->getPrimaryKey() as $col) {
            ...
            if ($table->isAllowPkInsert()) {
                $script .= "
        if (\$pk !== null) {
            \$this->set".$col->getName()."(\$pk);  //[IMV] update autoincrement primary key
        }";
            } else {
                $script .= "
        \$this->set".$col->getName()."(\$pk);  //[IMV] update autoincrement primary key";
        ...
        foreach ($table->getColumns() as $col) {
            ...
            if ($col->isForeignKey()) {
                foreach ($col->getForeignKeys() as $fk) {
                    ...
                    $script .= "
        if (\$this->".$varName." !== null && \$this->$clo !== \$this->".$varName."->get".$colFK->getName()."()) {
            \$this->$varName = null;
        }";
                } // foreach
            } /* if col is foreign key */
    ...
    {";
        ...
        foreach ($table->getColumns() as $col) {
            if (!in_array($col, $autoIncCols, true)) {
                $script .= "
        \$copyObj->set".$col->getName()."(\$this->get".$col->getName()."());";
            }
        } // foreach
...
";
        ...
        foreach ($autoIncCols as $col) {
                ...
                $script .= "
            \$copyObj->set".$col->getName() ."($coldefval); // this is a auto-increment column, so set to default value";
        } // foreach
        $script .= "
        }
    }
...
";
    ...
    {
        return (string) \$this->get{$column->getName()}();
    }
";
