 ...
 */
class OldQueryBuilder extends OldAbstractOMBuilder
{
    /**
    ...
    {
        return 'Base' . $this->getStubQueryBuilder()->getUnprefixedClassName();
    }
 ...
 *";
        ...
        foreach ($this->getTable()->getColumns() as $column) {
            $script .= "
 * @method     $queryClass orderBy" . $column->getName() . "(\$order = Criteria::ASC) Order by the " . $column->getName() . " column";
        }
        $script .= "
 *";
        ...
        foreach ($this->getTable()->getColumns() as $column) {
            $script .= "
 * @method     $queryClass groupBy" . $column->getName() . "() Group by the " . $column->getName() . " column";
        }
 ...
 *";
        ...
        foreach ($this->getTable()->getColumns() as $column) {
            $script .= "
 * @method     $modelClass findOneBy" . $column->getName() . "(" . $column->getPhpType() . " \$" . $column->getName() . ") Return the first $modelClass filtered by the " . $column->getName() . " column";
        }
        $script .= "
 *
 * @method     {$modelClass}[]|ObjectCollection find(ConnectionInterface \$con = null) Return $modelClass objects based on current ModelCriteria";
        foreach ($this->getTable()->getColumns() as $column) {
            $script .= "
 * @method     {$modelClass}[]|ObjectCollection findBy" . $column->getName() . "(" . $column->getPhpType() . " \$" . $column->getName() . ") Return $modelClass objects filtered by the " . $column->getName() . " column";
        }
...
";
    ...
    protected function addConstructorOpen(&$script)
    {
        $table = $this->getTable();
        $script .= "
    public function __construct(\$dbName = '" . $table->getDatabase()->getName() . "', \$entityName = '" . addslashes($this->getNewStubObjectBuilder($table)->getFullyQualifiedClassName()) . "', \$entityAlias = null)
    {";
    }
...
";
     ...
     *
     * @param     string \$entityAlias The alias of a model in the query
...
";
        ...
        if (\$this->formatter || \$this->entityAlias || \$this->with || \$this->select
...
";
    ...
    public function filterBy$relationName($objectName, \$comparison = null)
    {
        ...
        foreach ($fk->getLocalForeignMapping() as $localColumn => $foreignColumn) {
            ...
            $script .= "
                ->addUsingAlias(" . $this->getColumnConstant($localColumnObject) . ", " . $objectName . "->get" . $foreignColumnObject->getName() . "(), \$comparison)";
        }
        $script .= ";";
        if (!$fk->isComposite()) {
            $localColumnConstant = $this->getColumnConstant($fk->getLocalColumn());
            $foreignColumnName = $fk->getForeignColumn()->getName();
...
";
    ...
    protected function addUseRelatedQuery(&$script, Table $fkTable, $queryClass, $relationName, $joinType)
    {
        $script .= "
    /**
     * Use the $relationName relation " . $fkTable->getName() . " object
     *
     * @see useQuery()
     *
     * @param     string \$relationAlias optional alias for the relation,
...
";
    ...
    protected function addFilterByCrossFK(&$script, CrossForeignKeys $crossFKs)
    {
        ...
        foreach ($crossFKs->getCrossForeignKeys() as $crossFK) {
            ...
            $fkPhpName =  $foreignTable->getName();
    ...
    public function prune($objectName = null)
    {
        ...
        if (count($pks) > 1) {
            ...
            foreach ($pks as $col) {
                ...
                $conditions[]= $condName;
                $script .= "
            \$this->addCond(". $condName . ", \$this->getAliasedColName($const), " . $objectName . "->get" . $col->getName() . "(), Criteria::NOT_EQUAL);";
                $i++;
            }
            ...
            $const = $this->getColumnConstant($col);
            $script .= "
            \$this->addUsingAlias($const, " . $objectName . "->get" . $col->getName() . "(), Criteria::NOT_EQUAL);";
        } else {
            $this->declareClass('Propel\\Runtime\\Exception\\LogicException');
            $script .= "
        ...
        foreach ($table->getReferrers() as $fk) {
            ...
            ";
                    for ($x = 0, $xlen = count($columnNamesF); $x < $xlen; $x++) {
                        ...
                        $script .= "
            \$query->add(".$joinedTableTableMapBuilder->getColumnConstant($columnFK) .", \$obj->get".$columnL->getName()."());";
                    }
        ...
        foreach ($table->getReferrers() as $fk) {
            ...
            \$updateValues = new Criteria();";
                    ...
                    for ($x = 0, $xlen = count($columnNamesF); $x < $xlen; $x++) {
                        ...
                        $columnL = $table->getColumn($columnNamesL[$x]);
                        $script .= "
            \$query->add(".$refTableTableMapBuilder->getColumnConstant($columnFK).", \$obj->get".$columnL->getName()."());
