...
class MigrationManager extends AbstractManager
{
    ...
    public function getAllDatabaseVersions(): array
    {
        ...
        usort($migrationTimestamps, function (array $a, array $b) {
            if ($a['execution_datetime'] === $b['execution_datetime']) {
                return $a['version'] <=> $b['version'];
            }

            return $a['execution_datetime'] <=> $b['execution_datetime'];
        });

        return array_map(function (array $migrationTimestamp) {
            return (int)$migrationTimestamp['version'];
        }, $migrationTimestamps);
    }
    ...
    {
        if ($this->columnExists($connection, 'execution_datetime')) {
            return;
        }
