...
abstract class Media implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Propel\\Tests\\Bookstore\\Map\\MediaTableMap';
    ...
    /**
     * Whether the lazy-loaded $cover_image value has been loaded from database.
    ...
    /**
     * Initializes internal state of Propel\Tests\Bookstore\Base\Media object.
     */
     ...
     * @return resource
     */
    public function getCoverImage(ConnectionInterface $con = null)
    {
        if (!$this->cover_image_isLoaded && $this->cover_image === null && !$this->isNew()) {
            $this->loadCoverImage($con);
        }
    ...
    /**
     * Load the value for the lazy-loaded [cover_image] column.
     *
     * This method performs an additional query to return the value for
     * the [cover_image] column, since it is not populated by
     * the hydrate() method.
     *
     ...
     */
    protected function loadCoverImage(ConnectionInterface $con = null)
    {
        $c = $this->buildPkeyCriteria();
        $c->addSelectColumn(MediaTableMap::COL_COVER_IMAGE);
        try {
            $dataFetcher = ChildMediaQuery::create(null, $c)->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
        ...
        $firstColumn = $row ? current($row) : null;
            ...
            if ($firstColumn !== null) {
                $this->cover_image = fopen('php://memory', 'r+');
                fwrite($this->cover_image, $firstColumn);
                rewind($this->cover_image);
            } else {
                $this->cover_image = null;
            }
            $this->cover_image_isLoaded = true;
        } catch (Exception $e) {
            throw new PropelException("Error loading value for [cover_image] column on demand.", 0, $e);
        }
    }
    /**
     ...
     * @return string
     */
    public function getExcerpt(ConnectionInterface $con = null)
    {
        if (!$this->excerpt_isLoaded && $this->excerpt === null && !$this->isNew()) {
            $this->loadExcerpt($con);
        }
    ...
    /**
     * Load the value for the lazy-loaded [excerpt] column.
     *
     * This method performs an additional query to return the value for
     * the [excerpt] column, since it is not populated by
     * the hydrate() method.
     *
     * @param      $con ConnectionInterface (optional) The ConnectionInterface connection to use.
     * @return void
     * @throws PropelException - any underlying error will be wrapped and re-thrown.
     */
    protected function loadExcerpt(ConnectionInterface $con = null)
    {
        $c = $this->buildPkeyCriteria();
        $c->addSelectColumn(MediaTableMap::COL_EXCERPT);
        try {
            $dataFetcher = ChildMediaQuery::create(null, $c)->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
            $row = $dataFetcher->fetch();
            $dataFetcher->close();
        ...
        $firstColumn = $row ? current($row) : null;
            ...
            $this->excerpt = ($firstColumn !== null) ? (string) $firstColumn : null;
            $this->excerpt_isLoaded = true;
        } catch (Exception $e) {
            throw new PropelException("Error loading value for [excerpt] column on demand.", 0, $e);
        }
    }
    /**
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : MediaTableMap::translateFieldName('BookId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = MediaTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Propel\\Tests\\Bookstore\\Media'), 0, $e);
        }
    }
     ...
     * @return $this|\Propel\Tests\Bookstore\Media The current object (for fluent API support)
     */
    ...
    public function setCoverImage($v)
    {
        ...
        // when the getCoverImage() method is called.
        $this->cover_image_isLoaded = true;
        ...
        if (!is_resource($v) && $v !== null) {
            $this->cover_image = fopen('php://memory', 'r+');
            fwrite($this->cover_image, $v);
            rewind($this->cover_image);
        } else { // it's already a stream
            $this->cover_image = $v;
        }
        $this->modifiedColumns[MediaTableMap::COL_COVER_IMAGE] = true;
    ...
    public function setExcerpt($v)
    {
        ...
        // it doesn't matter if the value is actually set or not (logic below) as
        // any attempt to set the value means that no db lookup should be performed
        // when the getExcerpt() method is called.
        $this->excerpt_isLoaded = true;
        ...
        if ($this->excerpt !== $v) {
            $this->excerpt = $v;
            $this->modifiedColumns[MediaTableMap::COL_EXCERPT] = true;
        }
    ...
    public function setBookId($v)
    {
        ...
        if ($this->book_id !== $v) {
            $this->book_id = $v;
            $this->modifiedColumns[MediaTableMap::COL_BOOK_ID] = true;
        }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        // Reset the cover_image lazy-load column
        $this->cover_image = null;
        $this->cover_image_isLoaded = false;

        // Reset the excerpt lazy-load column
        $this->excerpt = null;
        $this->excerpt_isLoaded = false;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->isNew() || $this->isModified()) {
                ...
                $affectedRows += 1;
                // Rewind the cover_image LOB column, since PDO does not rewind after inserting value.
                if ($this->cover_image !== null && is_resource($this->cover_image)) {
                    rewind($this->cover_image);
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MediaTableMap::COL_COVER_IMAGE)) {
            $modifiedColumns[':p' . $index++]  = 'COVER_IMAGE';
        }
        if ($this->isColumnModified(MediaTableMap::COL_EXCERPT)) {
            $modifiedColumns[':p' . $index++]  = 'EXCERPT';
        }
        if ($this->isColumnModified(MediaTableMap::COL_BOOK_ID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'ID':
                        ...
                        break;
                    case 'COVER_IMAGE':
                        if (is_resource($this->cover_image)) {
                            rewind($this->cover_image);
                        }
                        $stmt->bindValue($identifier, $this->cover_image, PDO::PARAM_LOB);
                        break;
                    case 'EXCERPT':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => ($includeLazyLoadColumns) ? $this->getCoverImage() : null,
            $keys[2] => ($includeLazyLoadColumns) ? $this->getExcerpt() : null,
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Propel\Tests\Bookstore\Media
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setCoverImage($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setExcerpt($arr[$keys[2]]);
        }
     ...
     *
     * @return $this|\Propel\Tests\Bookstore\Media The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MediaTableMap::COL_COVER_IMAGE)) {
            $criteria->add(MediaTableMap::COL_COVER_IMAGE, $this->cover_image);
        }
        if ($this->isColumnModified(MediaTableMap::COL_EXCERPT)) {
            $criteria->add(MediaTableMap::COL_EXCERPT, $this->excerpt);
        }
        if ($this->isColumnModified(MediaTableMap::COL_BOOK_ID)) {
            $criteria->add(MediaTableMap::COL_BOOK_ID, $this->book_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Propel\Tests\Bookstore\Media (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Propel\Tests\Bookstore\Media Clone of current object.
     * @throws PropelException
     */
