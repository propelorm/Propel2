...
/**
 * Base class that represents a row from the 'contest' table.
 *
 *
 *
...
abstract class Contest implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Propel\\Tests\\Bookstore\\Map\\ContestTableMap';
    ...
    /**
     * Initializes internal state of Propel\Tests\Bookstore\Base\Contest object.
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            return $startcol + 2; // 2 = ContestTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Propel\\Tests\\Bookstore\\Contest'), 0, $e);
        }
    }
     ...
     * @return $this|\Propel\Tests\Bookstore\Contest The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ContestTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[ContestTableMap::COL_NAME] = true;
        }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ContestTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(ContestTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'NAME';
        }

        $sql = sprintf(
            'INSERT INTO contest (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['Contest'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Contest'][$this->getPrimaryKey()] = true;
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Propel\Tests\Bookstore\Contest
     */
     ...
     *
     * @return $this|\Propel\Tests\Bookstore\Contest The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ContestTableMap::COL_ID)) {
            $criteria->add(ContestTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ContestTableMap::COL_NAME)) {
            $criteria->add(ContestTableMap::COL_NAME, $this->name);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Propel\Tests\Bookstore\Contest (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Propel\Tests\Bookstore\Contest Clone of current object.
     * @throws PropelException
     */
    ...
    public function setBookstoreContests(Collection $bookstoreContests, ConnectionInterface $con = null)
    {
        ...
        foreach ($bookstoreContestsToDelete as $bookstoreContestRemoved) {
            $bookstoreContestRemoved->setContest(null);
        }
    ...
    protected function doAddBookstoreContest(ChildBookstoreContest $bookstoreContest)
    {
        ...
        $bookstoreContest->setContest($this);
    }
    ...
    public function removeBookstoreContest(ChildBookstoreContest $bookstoreContest)
    {
        if ($this->getBookstoreContests()->contains($bookstoreContest)) {
            ...
            $bookstoreContest->setContest(null);
        }
