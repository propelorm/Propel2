...
class DataFetcherTest extends BookstoreEmptyTestBase
{
    ...
    public function testArrayDataFetcher()
    {
        ...
        $this->assertNull($dataFetcher2->fetchColumn()); //no rows left, returns NULL
        $this->assertNull($dataFetcher2->fetchColumn()); //be sure further calls returns NULL as well

        $dataFetcher2->close();
        $this->assertNull($dataFetcher2->fetchColumn());

        $dataFetcher3 = new ArrayDataFetcher($items);
        $dataFetcher3->close();
        $this->assertNull($dataFetcher3->fetch());
        $this->assertNull($dataFetcher3->fetchColumn());
    }
}
